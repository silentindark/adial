...
exten => _X.,1,NoOp(Dialer Outbound: ${EXTEN} via ${TRUNK})
 ...
 same => n,Set(CDR(userfield)=${CAMPAIGN_ID}:${NUMBER_ID}:${AGENT_DEST})
 ...
 same => n,Set(__AGENT_DEST=${AGENT_DEST})
 ...
 same => n,Set(RECORDING_PATH=/var/spool/asterisk/monitor/dialer/${YEAR}/${MONTH}/${DAY}/${CALLFILENAME}.wav)
 ...
 same => n,Dial(${TRUNK}/${EXTEN},${DIAL_TIMEOUT},g)
...
exten => _X.,1,NoOp(Dialer Agent: Connecting to ${CHANNEL_TYPE}/${EXTEN})
 ...
 same => n,Set(__AGENT_EXTEN=${EXTEN})
 ...
 same => n,Dial(${CHANNEL_TYPE}/${EXTEN},${DIAL_TIMEOUT},gL(${CALL_TIMEOUT_MS}))
...
exten => _X.,1,NoOp(Dialer Queue: Adding to queue ${EXTEN})
 ...
 same => n,Set(CALLERID(num)=${DIALED_NUMBER})
 same => n,Set(CALLERID(name)=${DIALED_NAME})
 same => n,Set(__QUEUE_NAME=${EXTEN})
 same => n,Set(__NODEST=${EXTEN})
 same => n,Set(QAGENT=${EXTEN})
 same => n,Set(__FROMQ=true)
